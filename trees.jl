...
function node_operator(measure::pauli_measure, idx, mat, nod, branch)
    A = measure.proj[branch]*mat*measure.proj[branch]
    #println(A)
    pA = tr(A)
    if pA == 0
        node = TreeNode(idx*2+(branch-1), nod, density_prob(pA, zero(A), 0), nothing, nothing)
    else
        node = TreeNode(idx*2+(branch-1), nod, density_prob(pA, A./pA, measure.value[1]), nothing, nothing)
    end
    return node
end

function node_operator(chan::channel, idx, mat, nod, branch)
    return TreeNode(idx*2 +(branch-1), nod, density_prob(branch-1, chan.mat*mat*adjoint(chan.mat), 0), nothing, nothing)
end
...
function create_tree(meas::Vector{operators}, density_mat::density_matrix; start = 1, parent = nothing, prob = 1, res = 0)
    root = TreeNode(start, parent, density_prob(prob, density_mat.mat, res), nothing, nothing)
